...
PlayState.create = function () {
    ...
    console.log(dgameno);
    if (dgameno == 1) {
        this.addHiddenObject('1', 122, 167);
    } 
    if (dgameno == 2) {
        this.addHiddenObject('1', 269, 282);
    }
    this.addHiddenObject('2', 0, 0);

    document.getElementById('text').innerHTML = 'Click to begin';
}
...
PlayState.clickObject = function (hiddenObj) {
    ...
    if (hiddenObj.name == "hidden_2" && hiddenObj.visible) {
        document.getElementById('text').innerHTML = 'Click the odd one out!';
        stopwatch.start();
    } 
    if (hiddenObj.name == "hidden_1") {
        stopwatch.stop();
        document.getElementById('text').innerHTML = 'You found it!';
    } 
